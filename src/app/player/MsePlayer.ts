import { BasePlayer, DecodeType } from './BasePlayer';
...
export class MsePlayer extends BasePlayer {
    ...
    public static readonly decodeType: DecodeType = 'hardware';
    ...
    private static readonly mobilePreferredVideoSettings: VideoSettings = new VideoSettings({
        lockedVideoOrientation: -1,
        bitrate: 3145728,
        maxFps: 15,
        iFrameInterval: 5,
    ...
    private MAX_TIME_TO_RECOVER = this.isMobile ? 100 : 200; // ms
    private MAX_BUFFER = this.isSafari ? 2 : this.isChrome && this.isMac ? 0.9 : this.isMobile ? 0.1 : 0.2;
    private MAX_AHEAD = -0.2;
    private static readonly FPS_WINDOW = 30;
    private static readonly FPS_DRIFT_THRESHOLD = 0.2;
    ...
    public play(): void {
        ...
        if (!this.converter) {
            this.converter = MsePlayer.createConverter(this.tag, this.getEffectiveFps(), this.fpf);
            this.canPlay = false;
            this.resetStats();
        }
        this.converter.play();
    ...
    public getPreferredVideoSetting(): VideoSettings {
        return this.isMobile ? MsePlayer.mobilePreferredVideoSettings : MsePlayer.preferredVideoSettings;
    }
    ...
    cleanSourceBuffer = (): void => {
        ...
        const minBlocks = this.isMobile ? 5 : 10;
        if (this.blocks.length < minBlocks) {
            return;
        }
        try {
    ...
    private measureFps(): void {
        ...
        if (this.frameTimestamps.length > MsePlayer.FPS_WINDOW) {
            this.frameTimestamps.shift();
        }
        if (this.frameTimestamps.length >= 2) {
            const elapsed = this.frameTimestamps[this.frameTimestamps.length - 1] - this.frameTimestamps[0];
            if (elapsed > 0) {
                this.measuredFps = ((this.frameTimestamps.length - 1) / elapsed) * 1000;
            }
        }
    }

    private getEffectiveFps(): number {
        if (this.vsync && this.measuredFps > 0) {
            return Math.round(this.measuredFps);
        }
    ...
    private needsFpsResync(): boolean {
        if (!this.vsync || !this.converter || this.measuredFps === 0) {
            return false;
        }
        const currentFps = this.getEffectiveFps();
        const drift = Math.abs(this.measuredFps - currentFps) / currentFps;
        return drift > MsePlayer.FPS_DRIFT_THRESHOLD;
    }

    public pushFrame(frame: Uint8Array): void {
        super.pushFrame(frame);
        this.measureFps();
        if (this.needsFpsResync()) {
            const state = this.getState();
            this.stop();
            this.converter = MsePlayer.createConverter(this.tag, this.getEffectiveFps(), this.fpf);
            this.canPlay = false;
    ...
    protected checkForIFrame(frame: Uint8Array): boolean {
        ...
        if (BasePlayer.isIFrame(frame)) {
            ...
            if (end !== 0 && start < end) {
                ...
                if (this.blocks.length > (this.isMobile ? 5 : 10)) {
